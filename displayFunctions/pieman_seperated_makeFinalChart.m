...
results = separated_pieman_isc(structToVolume(piemandata, 1:32, 1), structToVolume(piemandata, 1:32, 0), structToVolume(piemandata, 1:32, 2), 1:12, 13:32);
...
[h,p2]=ttest2(results.persub_Wfb_f(1:12), results.persub_Wfb_f(13:end));
[h,p3]=ttest2(results.persub_Wfb_b(1:13), results.persub_Wfb_b(13:end));
[h,p5]=ttest2([results.persub_Wfb_f(1:12)+results.persub_Wfb_b(1:12)], [results.persub_Wfb_f(13:end)+results.persub_Wfb_b(13:end)]);
[h, p6] = ttest(results.persub_Wfb_f(1:12) - results.persub_Wfb_b(1:12));
[h, p7] = ttest(results.persub_Wfb_f(13:end) - results.persub_Wfb_b(13:end));
subplot(1,2,1)
...
for i = 1:3
  subplot(3,2,i*2);
  topoplot(results.A_fs(:,i),'extra_EOG.loc','electrodes','off'); title(['Component ' num2str(i) ''])
end

[h,p2]=ttest2(results.persub_Wfs_f(1:12), results.persub_Wfs_f(13:end));
[h,p3]=ttest2(results.persub_Wfs_s(1:13), results.persub_Wfs_s(13:end));
[h,p5]=ttest2([results.persub_Wfs_f(1:12)+results.persub_Wfs_s(1:12)], [results.persub_Wfs_f(13:end)+results.persub_Wfs_s(13:end)]);
[h, p6] = ttest(results.persub_Wfs_f(1:12) - results.persub_Wfs_s(1:12));
[h, p7] = ttest(results.persub_Wfs_f(13:end) - results.persub_Wfs_s(13:end));
subplot(1,2,1)
sigstar({[1 3] [2 4] [1 4] [1 2] [3 4]}, [p2 p3 p5 p6 p7])
