% -------------------------------------------------------------------------
function X = preprocessEEG(X,eogchannels,badchannels,fs)
...
% add 1s before and after data to counteract filter transient
data = cat(1, zeros(fs, D), data, zeros(fs, D));

% high-pass filter
data = sosfilt(sos,data);

% remove 1s before and after
data(1:fs,:) = [];
data(T+1:end,:) = [];
